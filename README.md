# ball-escape
 
### Useful Links

#### libGDX Resources
* [libGDX Wiki](https://libgdx.com/wiki/)
* [Box2D Documentation](https://libgdx.com/wiki/extensions/physics/box2d)
* [Box2DLights Documentation](https://github.com/libgdx/box2dlights/wiki)
* [noise4j procedural map generation library](https://github.com/czyzby/noise4j)

#### Design
* [Tiled Level Editor](https://www.mapeditor.org/)
* [Piskel pixel art editor](https://www.piskelapp.com/)
* [libGDX GUI texture packer](https://github.com/crashinvaders/gdx-texture-packer-gui)
* [Skin Composer](https://github.com/raeleus/skin-composer)
* [libGDX Tile Maps documentation](https://libgdx.com/wiki/graphics/2d/tile-maps)
